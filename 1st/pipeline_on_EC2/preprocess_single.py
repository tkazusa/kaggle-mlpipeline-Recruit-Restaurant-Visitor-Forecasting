...
def get_store_exp_visitor_feat(label, key, n_day):
    ...
    result["store_exp_mean{}".format(n_day)] = (
        result["store_exp_mean{}".format(n_day)]
...
def get_store_week_exp_feat(label, key, n_day):
    ...
    for i in [0.9, 0.95, 0.97, 0.98, 0.985, 0.99, 0.999, 0.9999]:
        ...
        result1 = data_temp.groupby(["store_id", "dow"], as_index=False)[
            "visitors1"
        ].agg({"store_dow_exp_mean{}_{}".format(n_day, i): "sum"})
        result3 = data_temp.groupby(["store_id", "dow"], as_index=False)[
            "visitors2"
        ].agg({"store_dow_exp_mean2{}_{}".format(n_day, i): "sum"})
...
def get_store_holiday_feat(label, key, n_day):
    ...
    start_date = date_add_days(key[0], -n_day)
    data_temp = data[(data.visit_date < key[0]) & (data.visit_date > start_date)].copy()
    result1 = data_temp.groupby(["store_id", "holiday_flg"], as_index=False)[
        "visitors"
    ].agg(
        {
    ...
    result2 = data_temp.groupby(["store_id", "holiday_flg2"], as_index=False)[
        "visitors"
    ].agg(
        {
...
def get_genre_exp_visitor_feat(label, key, n_day):
    ...
    result["genre_exp_mean{}".format(n_day)] = (
        result["genre_exp_mean{}".format(n_day)]
...
def get_genre_week_feat(label, key, n_day):
    ...
    result = genre_week["visitors"].agg(
        {
...
def get_genre_week_exp_feat(label, key, n_day):
    ...
    result1 = data_temp.groupby(["air_genre_name", "dow"], as_index=False)[
        "visitors"
    ].agg({"genre_dow_exp_mean{}".format(n_day): "sum"})
    result2 = data_temp.groupby(["air_genre_name", "dow"], as_index=False)[
        "weight"
    ].agg({"genre_dow_exp_weight_sum{}".format(n_day): "sum"})
    result = result1.merge(result2, on=["air_genre_name", "dow"], how="left")
    result["genre_dow_exp_mean{}".format(n_day)] = (
        result["genre_dow_exp_mean{}".format(n_day)]
...
def get_air_reserve_feat(label, key, label_end_date):
    ...
    air_reserve_temp = air_reserve[
        (air_reserve.visit_date >= key[0])
        & (air_reserve.visit_date < label_end_date)  # key[0] 是'2017-04-23'
        & (air_reserve.reserve_date < key[0])  # label_end_date 是'2017-05-31'
    ].copy()
    ...
    air_store_diff_time_mean = left_merge(
        label, air_store_diff_time_mean, on=["store_id", "visit_date"]
    ).fillna(0)
...
def get_hgp_reserve_feat(label, key, label_end_date):
    ...
    hpg_store_diff_time_mean = left_merge(
        label, hpg_store_diff_time_mean, on=["store_id", "visit_date"]
    ).fillna(0)
    ...
    hpg_diff_time_mean = left_merge(
        label, hpg_diff_time_mean, on=["visit_date"]
    ).fillna(0)

    result = pd.concat(
        [hpg_result, hpg_date_result, hpg_store_diff_time_mean, hpg_diff_time_mean],
        axis=1,
    )
    return result
...
if __name__ == "__main__":
    ...
    train_feat = make_feats(end_date, n_days)
    train_feat.to_csv("train_feat_single.csv", index=False, header=True)

    single_window_end = time.time()
    print("Feature Engineering Completed")
    print("Data Size: {}".format(train_feat.shape))
    print("Time elapsed {} sec".format(single_window_end - single_window_start))
    mem = psutil.virtual_memory()
    print(
        "memory usage total:{} usage: {} available: {}".format(
            mem.total, mem.used, mem.available
        )
    )

    print("Feature Engineering for Test Data Start")
    test_start = time.time()
    test_feat = make_feats(date_add_days(start_date, 42), 39)
    test_feat.to_csv("test_feat.csv", index=False, header=True)
    test_end = time.time()
    print("Time elapsed {} sec".format(test_end - test_start))
